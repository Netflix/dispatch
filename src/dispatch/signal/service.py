...
def create_instance(
    *, db_session: Session, signal_instance_in: SignalInstanceCreate
    ...
    if signal_instance.id and not is_valid_uuid(signal_instance.id):
        msg = f"Invalid signal id format. Expecting UUIDv4 format. Signal id: {signal_instance.id}. Signal name/variant: {signal_instance.raw['name'] if signal_instance and signal_instance.raw and signal_instance.raw['name'] else signal_instance.raw['variant']}"
        log.warn(msg)
        raise HTTPException(
