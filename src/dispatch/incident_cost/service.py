...
def get(*, db_session, incident_cost_id: int) -> IncidentCost | None:
    """Gets an incident cost by its id."""
    return db_session.query(IncidentCost).filter(IncidentCost.id == incident_cost_id).one_or_none()


def get_by_incident_id(*, db_session, incident_id: int) -> list[IncidentCost]:
...
def get_all(*, db_session) -> list[IncidentCost]:
...
) -> list[tuple[datetime.timestamp, str]]:
