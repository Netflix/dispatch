...
def upgrade():
    ...
    op.add_column("incident", sa.Column("primary_team", sa.String(), nullable=True))
    op.add_column("incident", sa.Column("primary_location", sa.String(), nullable=True))
    ...
    op.add_column("incident", sa.Column("liason_id", sa.Integer(), nullable=True))
    ...
    op.create_foreign_key(None, "incident", "participant", ["liason_id"], ["id"])
    ...
    for incident in incidents:
        ...
        incident.primary_team = Counter(p.team for p in incident.participants).most_common(1)[0][0]
        incident.primary_location = Counter(p.location for p in incident.participants).most_common(
            1
        )[0][0]
        ...
        liason = get_current_participant(incident.participants, ParticipantRoleType.liaison)
        if liason:
            incident.liason_id = liason.id
...
def downgrade():
    ...
    op.drop_column("incident", "liason_id")
