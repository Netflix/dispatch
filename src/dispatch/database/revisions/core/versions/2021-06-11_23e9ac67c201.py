...
Revision ID: 23e9ac67c201
Revises: f011c050b9ba
Create Date: 2021-06-11 14:44:56.592975
...
def upgrade():
    ...
    op.alter_column('dispatch_user_organization', 'dispatch_user_id',
               existing_type=sa.INTEGER(),
               nullable=False)
    op.alter_column('dispatch_user_organization', 'organization_id',
               existing_type=sa.INTEGER(),
               nullable=False)
...
def downgrade():
    ...
    op.add_column('dispatch_user_organization', sa.Column('id', sa.INTEGER(), server_default=sa.text("nextval('public.dispatch_user_organization_id_seq'::regclass)"), autoincrement=True, nullable=False))
    op.drop_constraint(None, 'dispatch_user_organization', schema='dispatch_core', type_='foreignkey')
    op.drop_constraint(None, 'dispatch_user_organization', schema='dispatch_core', type_='foreignkey')
    op.create_foreign_key('dispatch_user_organization_organization_id_fkey', 'dispatch_user_organization', 'organization', ['organization_id'], ['id'], referent_schema='public')
    op.create_foreign_key('dispatch_user_organization_dispatch_user_id_fkey', 'dispatch_user_organization', 'dispatch_user', ['dispatch_user_id'], ['id'], referent_schema='public')
    op.alter_column('dispatch_user_organization', 'organization_id',
               existing_type=sa.INTEGER(),
               nullable=True)
    op.alter_column('dispatch_user_organization', 'dispatch_user_id',
