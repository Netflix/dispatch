...
def get_all(*, db_session) -> list[Task]:
    """Return all tasks."""
    return db_session.query(Task).all()


def get_all_by_incident_id(*, db_session, incident_id: int) -> list[Task]:
    """Get all tasks by incident id."""
    return db_session.query(Task).filter(Task.incident_id == incident_id).all()


def get_all_by_incident_id_and_status(*, db_session, incident_id: int, status: str) -> list[Task]:
...
def get_overdue_tasks(*, db_session, project_id: int) -> list[Task]:
