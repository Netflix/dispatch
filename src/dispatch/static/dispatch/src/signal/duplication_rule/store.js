...
import DuplicationRuleApi from "@/signal/duplication_rule/api"
...
const actions = {
  getAll: debounce(({ commit, state }) => {
    ...
    return DuplicationRuleApi.getAll(params)
      .then((response) => {
  ...
  save({ commit, state, dispatch }) {
    ...
    if (!state.selected.id) {
      return DuplicationRuleApi.create(state.selected)
        .then(() => {
      ...
      return DuplicationRuleApi.update(state.selected.id, state.selected)
        .then(() => {
          commit("SET_SELECTED_LOADING", false)
          dispatch("closeCreateEdit")
          dispatch("getAll")
  ...
  remove({ commit, dispatch }) {
    return DuplicationRuleApi.delete(state.selected.id).then(function () {
