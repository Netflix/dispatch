<template>
  <v-dialog v-model="showRemove" persistent max-width="800px">
    <v-card>
      <v-card-title>
        <span class="text-h5">Delete Template?</span>
      </v-card-title>
      <v-card-text>
        <v-container> Are you sure you would like to delete this template? </v-container>
      </v-card-text>
      <v-card-actions>
        <v-spacer />
...
export default {
  ...
  computed: {
    ...mapFields("template", ["dialogs.showRemove"]),
  },

  methods: {
    ...mapActions("template", ["remove", "closeRemove"]),
  },
}
</script>
