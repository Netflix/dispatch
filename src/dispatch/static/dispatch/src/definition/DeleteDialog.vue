<template>
  <v-dialog v-model="showRemove" persistent max-width="800px">
    <v-card>
      <v-card-title>
        <span class="text-h5">Delete Definition?</span>
      </v-card-title>
      <v-card-text>
        <v-container grid-list-md>
          <v-layout wrap> Are you sure you would like to delete this definition? </v-layout>
        </v-container>
      </v-card-text>
      <v-card-actions>
        <v-spacer />
...
export default {
  ...
  computed: {
    ...mapFields("definition", ["dialogs.showRemove"]),
  },

  methods: {
    ...mapActions("definition", ["remove", "closeRemove"]),
  },
}
</script>
