  ...
  <div>
    <div class="pl-6 pr-8 d-flex justify-space-between align-center">
      <div class="button-group-container">
        <v-btn-toggle v-model="tab" mandatory variant="outlined" rounded="lg" selected-class="selected-button">
          <v-btn class="text-subtitle-2 unselected-button" height="24px" value="main" variant="plain" :ripple="false">
            <span class="button-text">Timeline</span>
          </v-btn>
          <v-btn class="text-subtitle-2 unselected-button" height="24px" value="signals" variant="plain" :ripple="false"
            :disabled="signalInstances?.length === 0">
            <span class="button-text">Alerts</span>
            <v-badge v-if="signalInstances?.length" inline :content="signalInstances?.length" class="small-badge" />
          </v-btn>
          <v-btn class="text-subtitle-2 unselected-button" height="24px" value="graph" variant="plain" :ripple="false"
            :disabled="signalInstances?.length === 0">
      ...
      <div>
        ...
        <DTooltip text="Open case ticket" hotkeys="">
          <template #activator="{ tooltip }">
            <v-btn class="text-subtitle-2 font-weight-regular" icon="mdi-jira" variant="text" size="small"
              :disabled="!modelValue.ticket" :href="modelValue.ticket && modelValue.ticket.weblink" target="_blank"
              v-bind="tooltip" />
          </template>
        </DTooltip>
        ...
        <DTooltip text="Open case conversation" hotkeys="">
          <template #activator="{ tooltip }">
            <v-btn class="text-subtitle-2 font-weight-regular" icon="mdi-slack" variant="text" size="small"
              :disabled="!modelValue.conversation" :href="modelValue.conversation && modelValue.conversation.weblink"
              target="_blank" v-bind="tooltip" />
          </template>
        </DTooltip>
        ...
        <DTooltip text="Open case document" hotkeys="">
          <template #activator="{ tooltip }">
            <v-btn class="text-subtitle-2 font-weight-regular" icon="mdi-file-document" variant="text" size="small"
              :disabled="!modelValue.documents.length"
              :href="modelValue.documents.length && modelValue.documents[0].weblink" target="_blank" v-bind="tooltip" />
          </template>
        </DTooltip>
        ...
        <DTooltip text="Open case storage" hotkeys="">
          <template #activator="{ tooltip }">
            <v-btn class="text-subtitle-2 font-weight-regular" icon="mdi-folder-google-drive" variant="text"
              size="small" :disabled="!modelValue.storage" :href="modelValue.storage && modelValue.storage.weblink"
              target="_blank" v-bind="tooltip" />
          </template>
        </DTooltip>
        ...
        <DTooltip text="Open legacy edit pane" hotkeys="">
          <template #activator="{ tooltip }">
            <v-btn class="text-subtitle-2 font-weight-regular" icon="mdi-square-edit-outline" variant="text"
              size="small" :disabled="loading" :href="editPageUrl()" target="_self" v-bind="tooltip" />
          </template>
        </DTooltip>
      </div>
    </div>
    ...
    <v-window v-model="tab">
      ...
      <v-window-item value="signals" class="tab">
        <case-signal-instance-tab :loading="loading" v-model="signalInstances" :selected-signal-id="selectedSignalId" />
      </v-window-item>
