...
<template>
    <div>
        ...
        <div class="d-flex flex-wrap gap-2 mb-2">
            <v-chip v-for="tag in selectedTags" :key="tag.id" variant="outlined" size="small" class="mr-1 linear-tag"
                @click="toggleTag(tag)" closable :close-icon="'mdi-close'" @click:close="toggleTag(tag)">
                <template #prepend>
                    <v-icon v-if="getTagIcon(tag)" :icon="getTagIcon(tag)" size="14" :color="getTagColor(tag)"
                        class="mr-1" />
                    <span v-else class="tag-dot" :style="`background-color: ${getTagColor(tag)}`"></span>
                </template>
                {{ tag.name }}
            </v-chip>

            <v-menu v-model="menu" :close-on-content-click="false" location="start" :offset-x="true" :position-x="-420"
                transition="false" :max-width="300" :min-width="300" attach="body" eager>
                <template #activator="{ props: menuProps }">
                    <v-btn icon="mdi-plus" variant="text" size="small" v-bind="menuProps" class="add-tag-button" />
                </template>

                <v-card width="300" class="rounded-lg">
                    <v-text-field v-model="searchQuery" density="compact" variant="solo" single-line hide-details flat
                        placeholder="Search tags..." prepend-inner-icon="mdi-magnify" :loading="loading" />
                    ...
                    <v-list density="compact">
                        ...
                        <v-list-item v-for="tag in tags" :key="tag.id" @click="toggleTag(tag)"
                            :active="isTagSelected(tag)" :class="{ 'selected-tag-item': isTagSelected(tag) }"
                            density="compact">
                            <template #prepend>
                                <v-checkbox :model-value="isTagSelected(tag)" :color="getTagColor(tag)"
                                    density="compact" hide-details />
                            </template>
                            <template #title>
                                <div class="d-flex align-center">
                                    <v-icon v-if="getTagIcon(tag)" :icon="getTagIcon(tag)" size="14"
                                        :color="getTagColor(tag)" class="mr-1" />
                                    <span v-else class="tag-dot mr-2"
