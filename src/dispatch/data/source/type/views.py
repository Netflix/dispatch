...
def get_source_type(*, db_session: Session = Depends(get_db), source_type_id: PrimaryKey):
    ...
    source = get(db_session=db_session, source_type_id=source_type_id)
    if not source:
        raise HTTPException(
...
def create_source_type(*, db_session: Session = Depends(get_db), source_type_in: SourceTypeCreate):
    ...
    source = create(db_session=db_session, source_type_in=source_type_in)
    return source
...
def update_source_type(
    *,
    ...
    source = update(db_session=db_session, source=source, source_type_in=source_type_in)
    return source
