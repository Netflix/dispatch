...
> Override where Dispatch should find the user email in the users claims (`x-amzn-oidc-data` header).
...
:::info
Make sure you add a listener action without authenticate for `/api/v1/{organization}/events/*`, if you want plugins to be reachable. Plugins determine their own authentication.
:::
